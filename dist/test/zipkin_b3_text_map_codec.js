...
describe('Zipkin B3 Text Map Codec should', function() {
  ...
  it('report a metric when failing to decode tracer state', function() {
    ...
    _chai.assert.isNotNull(context);
    _chai.assert.isTrue(_backend2.default.counterEquals(metrics.decodingErrors, 1));
  });
  ...
  it('return a context devoid of trace/span ids if invalid ids are encountered in the headers', function() {
    ...
    testCases.forEach(function(testCase) {
      var context = tracer.extract(opentracing.FORMAT_HTTP_HEADERS, testCase);

      _chai.assert.isNotNull(context);
      _chai.assert.equal('', context.spanIdStr);
      _chai.assert.equal('', context.traceIdStr);
      _chai.assert.equal('', context.parentIdStr);
    });
  });
  ...
  it("should normalize header's case and return a context", function() {
    ...
    _chai.assert.equal('123abc', context.traceIdStr);
    _chai.assert.equal('456def', context.spanIdStr);
    _chai.assert.equal('789ghi', context.parentIdStr);
    _chai.assert.isTrue(context.isSampled());
    _chai.assert.isTrue(context.isDebug());
    _chai.assert.equal('678pqr', context.debugId);
  });
