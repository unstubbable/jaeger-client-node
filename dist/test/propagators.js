...
describe('ZipkinB3TextMapCodec', function() {
  ...
  it('use an empty context if the zipkin headers contain invalid ids', function() {
    ...
    var ctx = codec.extract(carrier);
    _chai.assert.equal('', ctx.parentIdStr);
    _chai.assert.equal('', ctx.spanIdStr);
    _chai.assert.equal('', ctx.traceIdStr);
    _chai.assert.isTrue(ctx.isSampled());
    _chai.assert.isTrue(ctx.isDebug());
  });
