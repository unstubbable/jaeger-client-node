...
var _rate_limiting_sampler = require('../../src/samplers/rate_limiting_sampler.js');
...
describe('RateLimitingSampler should', function() {
  it('block after threshold is met', function() {
    ...
    var sampler = new _rate_limiting_sampler2.default(10, 10);
    for (var i = 0; i < 10; i++) {
    ...
    _chai.assert.isFalse(sampler.equal(new _probabilistic_sampler2.default(0.5)));
  ...
  it('should throw error when initialized with an incorrect value', function() {
    (0, _chai.expect)(function() {
      new _rate_limiting_sampler2.default(-2.0);
    }).to.throw('maxTracesPerSecond must be greater than 0.0.  Received -2');
  });

  it('should equal another rate limiting sampler', function() {
    var sampler = new _rate_limiting_sampler2.default(1.0);
    var otherSampler = new _rate_limiting_sampler2.default(1.0);

    _chai.assert.isTrue(sampler.equal(otherSampler));
  });

  it('work with maxCreditsPerSecond smaller than 1', function() {
    var initialDate = new Date(2011, 9, 1).getTime();
    var clock = _sinon2.default.useFakeTimers(initialDate);
    var sampler = new _rate_limiting_sampler2.default(0.1, 1);
  ...
  it('should update successfully', function() {
    var initialDate = new Date(2011, 9, 1).getTime();
    var clock = _sinon2.default.useFakeTimers(initialDate);
    var sampler = new _rate_limiting_sampler2.default(1.0, 1);
  ...
  it('should throw error when updated with an incorrect value', function() {
    var limiter = new _rate_limiting_sampler2.default(2.0);
    (0, _chai.expect)(function() {
      limiter.update(-2.0);
    }).to.throw('maxTracesPerSecond must be greater than 0.0.  Received -2');
  });
