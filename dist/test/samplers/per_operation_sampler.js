...
describe('PerOperationSampler', function() {
  ...
  it('should leave span non-finalized after first sampling, and finalize after setOperation', function() {
    var strategies = {
    ...
    var sampler = new _per_operation_sampler2.default(strategies, 2);
    var tracer = new _tracer2.default('test', new _noop_reporter2.default(), sampler);
    var span = tracer.startSpan('op1');
    _chai.assert.isFalse(span._spanContext.isSampled());
    _chai.assert.isFalse(span._spanContext.samplingFinalized);
  ...
  it('should have onSetTags', function() {
    var sampler = new _per_operation_sampler2.default(strategies, 0);
    var span = {};
    var decision = sampler.onSetTag(span, 'pi', 3.1415);
    _chai.assert.deepEqual(decision, { sample: false, retryable: true, tags: null });
  });
});
