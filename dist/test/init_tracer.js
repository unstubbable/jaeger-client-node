...
describe('initTracer', function() {
  it('should initialize noop tracer when disable is set', function() {
    ...
    (0, _chai.expect)(tracer).to.be.an.instanceof(opentracing.Tracer);
  });
  ...
  it('should initialize proper samplers', function() {
    ...
    var options = [
      ...
      { type: 'ratelimiting', param: 2, expectedType: _rate_limiting_sampler2.default, expectedParam: 2 },
      {
        type: 'probabilistic',
        param: 0.5,
    ...
    _lodash2.default.each(options, function(samplerConfig) {
      ...
      if (tracer._sampler._delegate) {
        (0, _chai.expect)(tracer._sampler._delegate).to.be.an.instanceof(expectedType);
      } else {
  ...
  describe('reporter options', function() {
    it('should respect reporter options', function(done) {
      ...
      _chai.assert.equal(remoteReporter._sender._socketType, 'udp6');
  ...
  it('should pass options to tracer', function(done) {
    ...
    _chai.assert.equal(tracer._traceId128bit, true);

    var textMapInjector = tracer._injectors[opentracing.FORMAT_TEXT_MAP];
