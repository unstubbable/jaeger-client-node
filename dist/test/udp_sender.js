...
function createUdpSenderTest(options) {
  describe(options.name, function() {
    ...
    beforeEach(function() {
      server = _dgram2.default.createSocket(options.socketType);
      ...
      sender = new _udp_sender2.default({ host: options.host, socketType: options.socketType });
        ...
        it('should serialize span references', function(done) {
          ...
          server.on('message', function(msg, remote) {
            ...
            if (o.expectedParentId) {
              ...
              _chai.assert.isUndefined(batch.spans[0].parentId);
            }
    ...
    it('should gracefully handle errors emitted by socket.send', function(done) {
      ...
      var ERROR_RE = /error sending spans over UDP: Error: getaddrinfo (EAI_AGAIN|ENOTFOUND|ESRCH)/;
      ...
      sender._logger = {
        ...
        error: function error(msg) {
          _chai.assert.isTrue(expectLogs);
          (0, _chai.expect)(msg).to.match(ERROR_RE);
          tracer.close(done);
        },
      };
      ...
      sender.flush(function(numSpans, err) {
        _chai.assert.equal(numSpans, 1);
        (0, _chai.expect)(err).to.match(ERROR_RE);
        if (!expectLogs) {
          tracer.close(done);
        }
      });
