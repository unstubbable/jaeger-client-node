...
describe('All Reporters should', function() {
  ...
  it('have proper toString', function() {
    var loggingReporter = new _logging_reporter2.default();
    var inMemoryReporter = new _in_memory_reporter2.default();
    inMemoryReporter.setProcess('service-name', []);
    var noopReporter = new _noop_reporter2.default();
    var remoteReporter = new _remote_reporter2.default(new _udp_sender2.default());
    var compositeReporter = new _composite_reporter2.default([loggingReporter, inMemoryReporter]);

    _chai.assert.equal(loggingReporter.toString(), 'LoggingReporter');
    _chai.assert.equal(inMemoryReporter.toString(), 'InMemoryReporter');
    _chai.assert.equal(noopReporter.toString(), 'NoopReporter');
    _chai.assert.equal(remoteReporter.toString(), 'RemoteReporter');
    _chai.assert.equal(compositeReporter.toString(), 'CompositeReporter(LoggingReporter,InMemoryReporter)');
  });
  ...
  closeOptions.forEach(function(o) {
    it('calls to close execute callback correctly', function() {
      ...
      _chai.assert.isTrue(o.predicate(o.callback));
    });
  });
  ...
  describe('Composite reporter', function() {
    it('should report spans', function() {
      ...
      _chai.assert.isTrue(mockReporter.report.calledOnce);
    });
  });
